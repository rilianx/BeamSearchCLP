  ...
  bool parallel=(atoi(argv[9])==1)? true:false;
	...
	sorted_lists.push_back(vlsl);
     ...
     //cout << double(best_volume) << endl;
         ...
         list<Action>::iterator it1=best_path.begin(); it1++;
         ...
         Block::calc=0;
          ...
          oss << output << "-" << inst << ".txt"; // write to string stream
         ...
         for(int i=1;best_path.end()!=it1; it1++,i++){
			 //~ cout << "block " << i << endl;
			 it1->block->print_loc(it1->x, it1->y,it1->z,myfile);
			 
		 }
         myfile << "volume:" << Block::calc << endl;
         myfile << "util(%):" << double(Block::calc) / double(clp.W*clp.L*clp.H) << endl;
         myfile.close();
...
void beam_search(CLP& clp, set<Block*, compareBlocks>& set_blocks, vector<SortedList*>& sorted_lists, 
    ...
    bool parallel_search){
		...
		list<Node*> S; 
		list<Node*> allNodes; //para segunda etapa->mejora, para encontrar camino de raiz a hojas (best_path)
		...
		while(S.size()!=0 && !clp.timeout()){
         ...
         /* 
					...
					for(;it_s!=succ.end() && !clp.timeout();it_s++){
						...
						if(new_fitness > best_volume){
						  best_volume=new_fitness; 
						  Node* nn=s;
						  best_path= s->path;

						  while(nn){
							  best_path.push_front(nn->action);
                              nn=nn->father;
						  }

					    }
						
						if(!time25 && clp.get_time()>45){
							time25=true;
							cout <<"Utilization(25):" << double(best_volume) / double(clp.W*clp.L*clp.H) << endl;
						}else if(!time50 && clp.get_time()>90){
							time50=true;
							cout <<"Utilization(50):" << double(best_volume) / double(clp.W*clp.L*clp.H) << endl;
						}else if(!time100 && clp.get_time()>170){
							time100=true;
		 ...
		 vector<SortedList*>& sorted_lists, double grasp){
		   ...
		   long64 max_fitness=cont.occupied_volume();
			 ...
			 if(depth==max_depth){
		  	 ...
		  	 set<FSpace, by_manhattan_distance>::iterator itsp=cont.free_spaces.begin();	
		  	 int nb_blocks=0;     
